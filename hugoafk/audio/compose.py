...
env = np.interp(np.arange(S.n_of(DUR)) / S.SR,
                [0, 0.6, 2.6, 3.0, 3.05, 20.5, 20.6, 23.5, 26.35, 26.36, 26.5, 26.55, 28.8, 29.6, 30],
...
# night room tone
music.add(S.stereo(S.lowpass(S.noise(DUR, 99, 'pink'), 1400) * np.interp(np.arange(S.n_of(DUR)) / S.SR, [0, 1, 26.3, 26.4, 26.5, 29, 30], [0, 1, 1, 0, 1, 1, 0])), 0, gain=0.007)
...
PAD = [  # (start, end, notes, gain, lowpass, attack)
    ...
    (15.0, 17.5, ['A2', 'C3', 'E3', 'A3'], 0.27, 2600, 0.30),
    (17.5, 20.5, ['D3', 'F3', 'A3', 'D4'], 0.27, 2600, 0.30),
    (20.5, 23.5, ['F3', 'A3', 'C4', 'F4', 'G4'], 0.30, 1500, 0.7),   # the calm trust beat
    (23.5, 26.35, ['D3', 'F3', 'A3', 'D4'], 0.20, 2400, 0.35),
    (26.5, 29.6, ['D3', 'F3', 'A3', 'E4', 'D4'], 0.30, 3000, 0.8),   # end card shimmer
]
for i, (a, b, ch, g, fc, att) in enumerate(PAD):
    x = S.pad([N(x) for x in ch], b - a + 1.4, attack=att, release=1.2, fc=fc, detune=7, seed=30 + i)
    if a >= 26.5: x = S.highpass(x, 180)
...
arp(17.5, 20.5, 0.40)
arp(23.5, 25.0, 0.42)
arp(25.0, 26.35, 0.46, octave=1, step=0.125)

# ------------------------------------------------------------------ bass
for a, b, root in ((3.0, 6.0, 'D2'), (6.0, 9.0, 'D2'), (9.0, 12.0, 'A#1'), (12.0, 15.0, 'C2'),
                   (15.0, 17.5, 'A1'), (17.5, 20.5, 'D2'), (23.5, 26.35, 'D2')):
    t, i = a, 0
    while t < b - 1e-6:
...
t = 25.0; i = 0
while t < 26.35 - 1e-6:
    add_snare(t, 0.14 + 0.40 * (t - 25.0) / 1.35); add_hat(t + BEAT / 16, 0.11, tone=10500, p=0.3 * (1 if i % 2 else -1))
    t += BEAT / 8; i += 1
# end card half-time kicks
for tt in (26.5, 27.5, 28.5):
...
for tt in (6.0, 9.0, 12.0, 15.0, 17.5):
    braam(tt, 1.8, 0.42)
...
sfx.add(S.reverb(S.sine(fdown, 0.5) * np.linspace(1, 0, n) ** 1.3, 1.6, 0.35), 15.4, gain=0.42)
sfx.add(S.glitch_stutter(0.3, N('D5'), seed=82, rate=1 / 16, bpm=BPM), 15.42, gain=0.30)
for i in range(5):
    sfx.add(S.tick(0.02, 1400, seed=90 + i), 15.95 + i * 0.1, gain=0.12)
n = S.n_of(0.45); fup = 200 * (900 / 200) ** np.linspace(0, 1, n)
sfx.add(S.reverb(S.sine(fup, 0.45) * np.linspace(0, 1, n) ** 0.6 * S.expdecay(0.45, 0.3), 1.8, 0.4), 16.4, gain=0.42)
for i, nm in enumerate(('D5', 'A5')):
    blip_up(16.9 + i * 0.09, N(nm), 0.30, 0.12)
# s07 phone: console ticks, a tap, restart confirmation
t, i = 17.7, 0
while t < 19.0:
    sfx.add(S.tick(0.015, 2000 + 140 * (i % 5), seed=110 + i), t, gain=0.10); t += 0.13; i += 1
sfx.add(S.reverb(blip8(N('F4'), 0.13, 0.25, seed=120), 1.2, 0.3), 19.0, gain=0.34)
sfx.add(S.kick(0.35, 110, 55, click=0.2, seed=121), 19.0, gain=0.35)
for i, nm in enumerate(('D5', 'A5')):
    blip_up(19.6 + i * 0.08, N(nm), 0.28, 0.11)
# s08 trust break: warm swell, seal assembly ticks, confirmation ping
sfx.add(S.reverse_cymbal(0.9, seed=130), 20.1, gain=0.35)
for i in range(8):
...
sfx.add(S.reverb(S.widen(x, 0.4), 2.6, 0.5), 20.9, gain=0.34)
for tt, nm in ((21.2, 'F5'), (21.6, 'A5'), (22.2, 'D5')):
...
sfx.add(S.sine(fr, 2.85) * np.linspace(0.2, 1, nb) ** 2, 23.5, gain=0.24)
sfx.add(S.sweep_filter(S.noise(2.85, 150), 200 * (12000 / 200) ** np.linspace(0, 1, nb) ** 1.5, 'high') * np.linspace(0.1, 1, nb) ** 2.5, 23.5, gain=0.42)
for i, tt in enumerate([25.0, 25.5, 25.75, 26.0, 26.125, 26.25]):
...
sfx.add(S.reverse_cymbal(0.85, seed=170), 25.5, gain=0.6)
whoosh(23.15, 0.35, 200, 6000, -0.8, 0.8, 0.4)
# s10 end card: final impact, pill thump, sweep shimmer
sfx.add(S.sub_drop(1.4, 95, 34), 26.5, gain=0.55)
fin = S.braam(3.4, N('D2'), attack=0.05, detune=(0, 5, -5, -1200, 1200, 1900, 2400), fc=(400, 3600), drive_amt=3.0, seed=180)
sfx.add(S.reverb(fin, 3.2, 0.34, damp=2800), 26.5, gain=0.9)
sfx.add(S.impact(2.6, 50, seed=181), 26.5, gain=0.6)
sfx.add(S.highpass(S.noise(0.25, 6) * S.expdecay(0.25, 0.04), 1600), 26.5, gain=0.45)
for i, nm in enumerate(('D5', 'A5', 'D6')):
    blip_up(26.95 + i * 0.1, N(nm), 0.30, 0.14)
sfx.add(S.reverb(S.highpass(S.noise(1.2, 9), 5000) * np.linspace(1, 0, S.n_of(1.2)) ** 2, 2.2, 0.5), 26.5, gain=0.16)
sfx.add(S.kick(0.45, 85, 44, click=0.05, seed=182), 27.9, gain=0.38)
sfx.add(S.tick(0.03, 3200, seed=183), 27.9, gain=0.18)
sfx.add(S.reverb(S.highpass(S.noise(0.45, 8), 6500) * np.sin(np.pi * np.linspace(0, 1, S.n_of(0.45))) ** 2, 1.6, 0.5), 28.6, gain=0.14)
whoosh(28.2, 0.9, 300, 2500, -0.5, 0.5, 0.09)
...
a, b = S.n_of(26.35), S.n_of(26.5); r = S.n_of(0.003)
...
print('gap 26.4-26.49 peak: %.4f' % float(np.max(np.abs(out[S.n_of(26.4):S.n_of(26.49)]))))
